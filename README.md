...
| `size` | string | 否 | OpenAI 风格尺寸：`1024x1024`/`1792x1024`/`1024x1792`/`1024x768`/`768x1024`；也可直接传比例字符串 |
| `ref_image_key` | string | 否 | 参考图片 key（图生图）。通过 `/v1/images/upload` 上传后获取 `uri` 字段 |

**图生图示例**（先上传参考图，再生成）：
```bash
# 1. 上传参考图片，获取 uri
URI=$(curl -s http://localhost:9090/v1/images/upload \
  -F "file=@ref.jpg" | jq -r '.uri')
